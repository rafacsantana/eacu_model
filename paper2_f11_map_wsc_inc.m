...
paramm=[5]; % 7 9 10 [1 2 3 4 5 6 10]; % rmsd
...
if plot_map
  ...
  for vari=1:length(variablem)
    ...
    for ex=1:length(expts)
      ...
      lt={'(a)','(b)','(c)','(d)','(e)','(f)'};
      title([lt{ex},' ',expt_name,': WSC analysis'],'fontsize',16,'fontweight','bold')% ,tname,' - mean=',num2str(nanmean(model(:)),'%.3f')]) 
      if ii==6
        title([lt{ex},' ',expt_name,': WSC JRA55-do'],'fontsize',16,'fontweight','bold')% ,tname,' - mean=',num2str(nanmean(model(:)),'%.3f')]) 
        HCB=colorbar;
        set(HCB,'position',[0.915 0.10975 0.0125661375661376 0.8152]) % [left bottom width height]
        set(get(HCB,'ylabel'),'string','Wind stres curl (WSC) (N/m^3)','fontsize',18,'fontweight','bold');
        %set(HCB,'ylabel','string','Along-slope velocity (m/s)','fontsize',fsize,'fontweight','bold');
      end

      v_spaa=1;
      m_vec(0.5,lon_avisom(1:v_spaa:end,1:v_spaa:end),lat_avisom(1:v_spaa:end,1:v_spaa:end),umod(1:v_spaa:end,1:v_spaa:end),vmod(1:v_spaa:end,1:v_spaa:end),'k','shaftwidth',.5,'headwidth',2,'headangle',45,'headlength',2.5) %,'shaftwidth',2,'headwidth',6,'headangle',60,'headlength',5)
      [hp ht]=m_vec(0.5,173.25,-37.5,.1,0,'k','shaftwidth',.5,'headwidth',2,'headangle',45,'headlength',2.5,'key',[num2str(0.1),' N/m^2']);
      set(ht,'fontsize',10,'fontweight','bold')

      %title([expt_name,' - ',tname,' - mean=',num2str(nanmean(model(:)),'%.3f')]) 
      m_plot(lonc_obs,latc_obs,'color',[1 0 1],'linewidth',2) 
      m_plot(lonc_obs,latc_obs,'.','color',[1 0 1],'markersize',14) 
    ...
    figname=[path_fig,'','ch3_fig11_map_wsc_inc','.png'];
