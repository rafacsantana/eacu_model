...
%runs=[79 81 82 95 98 99 102 103]; asfuv Nintervals 1440
%runs=[79 81 91 92 93 94 102 103]; % 3days
%runs=[79 81 101 104 97 99 100]; % aot2 
runs=[79 80 96 98 81]; % 
runs=[81 96 98 80 79]; 
runs=[137 141 142 136 79];



%runs=[79 83 95]; % asft2 
%runs=[82 83 89 91 92 94 95 96 97 98 99 100 101 102 103 104]; % still running
%runs=[84]; % quick check
cs=0
mooring=[4]
  ...
  for vari=1:length(variabletd)
    ...
    for m=mooring
      ...
      for ex=1:length(expts)
        ...
        if ex==1
          ...
          set(gca,'fontsize',10,'fontweight','bold') 
          title([lett{k},'M',num2str(m),' observed temperature (^oC)'])%,replace([varib],'_','-')]) 
          pcolor(time_lima,depth,obs); shading flat; 
          ...
          elseif strcmp(varib,'serie_mod_t') ||  strcmp(varib,'serie_diff_t')
            colormap(ax(k),avhrr); caxis([2 20])
          end

          %xlim([time_lima(1) time_lima(end)])
          mylim=[0 0 0 50 150];
          ylim([nanmin(dep_mod(:,ilatm(m),ilonm(m)))-mylim(m) 0])
          datetick('x','mmm/yy')
          set(gca,'xticklabel',[])
          %xlabel('2015-2016')
          ylabel('Depth (m)')
          ...
          h=colorbar;
          set(h,'position',[0.92    0.822665264814441        0.0222686151704941         0.102833161208644]) % [left bottom width height]

          if m==5 md=-700; else; md=0; end
          for tct=1:size(time_modcora,2)
            if mod(tct,2)==1 & tct~=27 & tct~=29
              text(time_modcora(1,tct),-900+md,num2str(tct),'color','c','fontsize',8,'fontweight','bold')
            elseif mod(tct,2)==0
              text(time_modcora(1,tct),-1000+md,num2str(tct),'color','r','fontsize',8,'fontweight','bold')
            elseif tct==27 
              text(time_modcora(1,tct),-800+md,num2str(tct),'color','b','fontsize',8,'fontweight','bold')
            elseif tct==29
              text(time_modcora(1,tct),-900+md,num2str(tct),'color','b','fontsize',8,'fontweight','bold')
            end
          end
        ...
        cax=-5:.5:6;
        rwb(33,:)=1; rwb(33,:)=1;
        for i=1:size(rwb,2); rwbi(:,i)=interp1(linspace(0,1,size(rwb,1)),rwb(:,i),linspace(0,1,length(cax))); end
        rwbi(round(length(rwbi)/2),:)=1;
        rwbi(round(length(rwbi)/2)-1,:)=1;
        %rwbi(round(length(rwbi)/2)-2,:)=1;
        %rwbi(round(length(rwbi)/2)+1,:)=1;
 
        %figure; imagesc(cax-.1); colorbar; caxis([cax(1)-.5 cax(end)]); colormap(rwbi)

        %imagesc(time_lima,dep_modg,model); shading flat; colorbar;
        pcolor(time_lima,dep_modg,model); shading flat; 
        %contourf(time_lima,dep_modg,model,[-5:5]); shading flat; colorbar;
        %contour(time_lima,dep_modg,model,[-5:5]); shading flat; colorbar;
        if strcmp(varib,'serie_mod_u') ||  strcmp(varib,'serie_diff_u') || strcmp(varib,'serie_mod_v') ||  strcmp(varib,'serie_diff_v')
        ...
        elseif strcmp(varib,'serie_diff_t')
          ...
          clabel(cs,h,'color',[0 0 0],'fontsize',10,'fontweight','bold','LabelSpacing',2000)
          colormap(ax(k),rwbi); caxis([cax(1)-.5 cax(end)])
        end
        title([lett{k},expt_name,' temp. diff. (model - obs)']) % ,' = ',num2str(nanmean(model),'%.3f')]}];
        %xlim([datenum(2015,5,11) datenum(2016,5,15)])
        %xlim([time_lima(1) time_lima(end)])
        ylim([nanmin(dep_mod(:,ilatm(m),ilonm(m)))-50 0])

        datetick('x','dd/mmm','keeplimits')
        if ex==length(expts)
          %datetick('x','mmm/yy')
          h=colorbar;
          set(h,'position',[0.92  0.11 0.0222686151704941         0.665]) % [left bottom width height]
    ...
    for ii=1:length(expts)+1
      ...
      for it=1:length(times)
        ...
        text(times(it)+5,mins(2),meson{it},'color','k','fontsize',10,'fontweight','bold')
    ...
    path_fig=['/scale_wlg_nobackup/filesets/nobackup/niwa00020/santanarc/figures/hikurangi/paper_2/'];
    figname=[path_fig,'','ch3_fig',num2str(m+1),'_td_m',num2str(m),'.png'];
    display(['Plotted: ',figname]);
    display(['PRESS ENTER TO SAVE FIG']);
