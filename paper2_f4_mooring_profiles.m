...
%time_lima  = datenum(2015,5,1,12,0,0):1:datenum(2017,3,31,12,0,0); % time of analysis
...
%runs=[79 81 84 89 85 86 109 110]; % m45
%runs=[79 91 94 102 92]; % 3days 
...
parampr=[3]; % um vm tm sm ru rv rt rs
%parampr=[5 6 7]; % um vm tm sm ru rv rt rs
parampr=[7 9 10 11]; % 12 correlations
parampr=[9 7]; % m3 
% time depth
...
colors={'m','c','r','b','k'};
colors={'r','b','c','m','k','y','g','--b','--m','--r','--g','--c','--k','--y','r','b','m','c','k','y','g'};
...
if plot_pr
  ...
  for vari=1:length(variablepr)
    ...
    for m=mooring
      mpr=mpr+1;
      mks={'d','.','s','^','p','.','.','^','p'};
      mss=[10 ,24 ,12 ,9  ,12 ,24 ,1  ,9  ,12];
      subplot(1,length(variablepr).*length(mooring),mpr)
      hold on
      for ex=1:length(expts)
        plot(0,-1500,'linestyle',lstyle{ex},'linewidth',2,'color',colors{ex},'marker',mks{ex},'MarkerFaceColor',colors{ex},'MarkerEdgeColor',colors{ex},'markersize',mss(ex))
      end
        ...
        for it=1:length(times)
          ...
          for ex=1:length(expts)
            ...
            elseif strcmp(varib,'v_complex_co') || strcmp(varib,'v_complex_an') 
              vname='Velocity complex correlation';
            ...
            set(gca,'fontsize',12,'fontweight','bold')
            lett={'(a)','(b)','(c)','(d)'};
            title([lett{kk},' M',num2str(m),' ',vname])%,' for ',meson{it}]) 

            if ~strcmp(varib,'serie_mod_ueof1') & ~strncmp(varib,'serie_rmsd_',11) & ~strncmp(varib,'v_complex_co',14) & ~strncmp(varib,'v_complex_an',14) & ~strncmp(varib,'serie_corr_',11)
            ...
            elseif strncmp(varib,'v_complex_co',14) ||  strncmp(varib,'v_complex_an',14)
              modelo=model;
              plot(model,dep_modg,'color',colors{ex},'linestyle',lstyle{ex},'linewidth',2)
              ...
              plot(modelo,depth,'color',colors{ex},'linestyle',lstyle{ex},'linewidth',2)
          ...
          if mpr==1
            ylabel('Depth (m)')
            xlabel('Complex correlation coef.')
