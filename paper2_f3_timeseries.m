...
%runs=[91 94 102 92]; % 3days 
%runs=[79 83 93 94 96 97 106]; % asfts 
%runs=[79 82 95 98 99 102 103 107]; % asfuv 
%runs=[79 82 95 98 107 83 96 97]; % asfts and asfuv 
%runs=[97 95 96 98]; % asfts and asfuv 
%runs=[85 86 109 110 111 112]; % still running 94:104 = 14 inner loops
%runs=[89 85 86 109 110]; % m45
%runs=[79 80 91 113 114]; % still running 94:104 = 14 inner loops
%runs=[114 115]; % still running 94:104 = 14 inner loops
c=0
mooring=[1]
...
parampr=[3 7]; % t bias and rmsd 
%parampr=[17 15 18 16]; % argo  
% time depth
paramtd=[7]; % u v t s du dv dt ds tc sc tcd scd
%paramtd=[1 7 8 11 12]; % part1
%paramtd=[11 12]; % u v t s du dv dt ds tc sc tcd scd
% series
params=[9:14];
params=[3 6 15 9];
params=[3 2 6 15];
%params=[14 7]; % divergence
%params=[2 3 4 6 9 15]; % part1
%params=[2 4 7 15]; % sst bias
%maps
paramm=[13];  % bias 
...
variablestd={'serie_mod_u' ,'serie_mod_v' ,'serie_mod_t'   ,'serie_mod_s',...
             ...
             't_modcora'   ,'s_modcora'   ,'t_modcora_diff','s_modcora_diff'};
...
variabless={'serie_mod_ssh','serie_corr_ssh','serie_rmsd_ssh',...
            ...
            'serie_rmsd_s' ,'serie_mod_div' ,'serie_bias_sst'};
...
if plot_pr
  ...
  figname=[path_fig,'','fig7_profiles.png'];
  ...
  for vari=1:length(variabletd)
    ...
    for m=mooring
      ...
      for ex=1:length(expts)
        ...
        ax(k)=subplot(length(expts)+1,1,k); hold on 
        if strncmp(varib,'t_modcora',9) || strncmp(varib,'s_modcora',9)
          scatter(time_corat(:),dept_corat(:),'filled','CData',model(:)); colorbar
        else
        ...
        elseif strcmp(varib,'serie_diff_s')
          [cs,h]=contour(time_lima,dep_modg,modelo,[35.4 35.4],'color',[0 0 0],'linewidth',1); 
          clabel(cs,h,'color',[0 0 0],'fontsize',10,'fontweight','bold','LabelSpacing',2000)
          colormap(ax(k),rwb); caxis([-2 2])

        elseif strcmp(varib,'t_modcora') 
          colormap(ax(k),cmocean('thermal')); caxis([2 23])

        elseif strcmp(varib,'t_modcora_diff') 
          colormap(ax(k),rwb); caxis([-3 3])

        elseif strcmp(varib,'s_modcora')
          colormap(ax(k),cmocean('haline')); caxis([34.4 35.8])

        elseif strcmp(varib,'s_modcora_diff') 
          colormap(ax(k),rwb); caxis([-1 1])
        end
...
if plot_ser
  ...
  %subp=[1,2,3;4,5,6;7,8,9];

  figure('position',scrsz,'color',[1 1 1],'visible','on');  hold on;


  time_tick=round((length(time_lima)*(time_lima(2)-time_lima(1)))/20);

  % Loop in the variables
  for vari=1:length(variables)

    %subplot((length(variables)+1)*2,1,subp(vari,:))
    subplot(8,1,subp(vari,:))
    set(gca,'fontsize',12,'fontweight','bold')
    hold on
    ...
    for ex=1:length(expts)
      ...
      if ex==1
        if strcmp(varib,'serie_mod_ssh')
          eval(['obs=serie_obs_ssh;']); obs=obs-nanmean(obs(:))+nanmean(model(:));
          plot(julian(datevec(time_lima)),obs,'color',green,'linewidth',4)
          lege=[lege,{['AVISO std = ',num2str(nanmean(obs),'%.3f')]}];
        elseif strcmp(varib,'serie_mod_sst')
          eval(['obs=serie_obs_sst;'])
          plot(julian(datevec(time_lima)),obs,'color',green,'linewidth',4)
          lege=[lege,{['AVHRR std = ',num2str(nanmean(obs),'%.3f')]}];
        elseif strcmp(varib,'serie_bias_sst')
          eval(['obs=serie_obs_sst;'])
          %yyaxis right
      ...
      if strcmp(varib,'serie_mod_t') || strcmp(varib,'serie_mod_s') || strcmp(varib,'serie_mod_ueof1') || strncmp(varib,'serie_rmsd_',11) & ~strcmp(varib,'serie_rmsd_sst') & ~strcmp(varib,'serie_rmsd_ssh')
        ...
        for m=mooring
          ...
          elseif strcmp(varib,'serie_mod_ueof1')
            ...
            model=squeeze(model(:,m,:));
            dep_modg=dep_mod(:,ilatm(m),ilonm(m));
            % selecting model data at the same depth as obs
            if m==5
               [~,i900]=nanmin(abs(dep_modg+900));
               dep_modg=dep_modg(i900:end,:);
               model=model(i900:end,:);
          ...
          %  hold on 
          %elseif m==5
          %  subplot((length(variables)+1)*2,1,subp(vari+1,:)); 
          %  set(gca,'fontsize',12,'fontweight','bold')
          %  hold on 
          %end
          ...
          if ex==1
            ...
            if ~strncmp(varib,'serie_rmsd_',11)
              plot(time_obser,obs,'color',green,'linewidth',4)
              if k==1
                lege=[lege,{['InSitu',', var=',num2str(nanvar(obs),'%.3f')]}];
              end
            end
          ...
          plot(julian(datevec(time_model)),model,colors{ex},'linewidth',2,'linestyle','-')
        end
        ...
        if strcmp(varib,'serie_mod_div')
          %model(isnan(model))=nanmean(model); model=fourfilt(model,length(model),time_lima(2)-time_lima(1),length(time_lima)/2,10).*1E7;
          model=model.*1E7;
        end

        plot(julian(datevec(time_lima)),model,colors{ex},'linewidth',2)

        if text_figs
          timemd; times=timem(1:end-1); timed=timed+.5;
          times=julian(datevec(times));
          timed=julian(datevec(timed));
          meson={'A1','C1','EAuC','A1','A2/C2','C2'};
          for it=1:length(times)
            [dif is]=nanmin(abs(time_lima-timem(it)));  [dif ie]=nanmin(abs(time_lima-timem(it+1))); 
            %text(nanmean(time_lima(is:ie)),nanmean(model(is:ie)),num2str(nanmean(model(is:ie)),'%.2f'),'color','k','fontsize',12,'fontweight','bold')
        ...
        %lege=[lege,{[expt_name,'-',num2str(runs(ex)),' = ',num2str(nanmean(model),'%.2f')]}];

        if strcmp(varib,'serie_bias_sst')
          lege=[lege,{[expt_name,'',' = ',num2str(nanstd(model),'%.2f')]}];
        else
          lege=[lege,{[expt_name,'',' = ',num2str(nanmean(model),'%.2f')]}];
        end
    ...
    if ~strcmp(varib,'serie_mod_t') & ~strcmp(varib,'serie_mod_s')
      xlim([julian(datevec(time_lima(1))) julian(datevec(time_lima(end)))])
      if vari==length(variables)
        %datetick('x','dd/mmm')
        %xlabel('2015-2016')
        gregaxd(julian(datevec(time_lima)),time_tick)
      else
        %set(gca,'xticklabel',[])
        set(gca,'xtick',[julian(datevec(time_lima(1)))+1.0:time_tick:julian(datevec(time_lima(end)))+1.0])
        set(gca,'xticklabel','')
        %grid
      end
      ylabel(replace([varib],'_','-'))
    ...
    if vari==4 & ex==3
      ...
      %  display(['Loading: ',fname])
      %  load(fname)
      %  plot(time_model,serie_bias_sst,colors{ex},'linewidth',2)
      %end
    end

    timemd; times=timem(1:end); timed=timed+.5;
    times=julian(datevec(times));
    timed=julian(datevec(timed));
    meson={'A1','C1','EAuC','A1','A2/C2','C2'};
    mins=get(gca,'Ytick');
    ...
    for it=1:length(times)-1
      ...
      text(times(it)+1,perc(vari),meson{it},'color','k','fontsize',12,'fontweight','bold')
      %plot(timed(it),mins(1),'.k','markersize',12)
    end
    plot([times(end) times(end)],[mins(1) mins(end)],'--k','linewidth',2)
    ...
    lh=legend(lege,'location','best','Orientation','vertical');
    if vari==4
      set(lh,'position',[0.648229167714404          0.11503683073958        0.0968749979045241        0.0827045655318304])
    elseif vari==1
      set(lh,'position',[0.648229167714404         0.844068144022222        0.0953124979510903        0.0770377712268715])
    elseif vari==2
      set(lh,'position',[0.648229167714404         0.555309066097199        0.0953124979510895         0.058648109732281])
    elseif vari==3
      set(lh,'position',[0.648229167714404          0.44365713855245          0.09531249795109        0.0770377712268716])
    end

%vari 2 0.663593751024455         0.548350815600181        0.0953124979510902         0.058648109732281

    if ex==3
      if vari==1
        title('(a) SSH: obs std and model rmsd')
        ylabel('(m)')
      elseif vari==2
        title('(b) model SSH spatial correlation with AVISO')
        ylabel('')
      elseif vari==3
        title('(c) SST: obs std and model rmsd')
        ylabel('(^oC)')
      elseif vari==4
        title('(d) SST: obs mean and model bias')
        ylabel('(^oC)')
      end
