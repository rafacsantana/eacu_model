...
time_lima  = datenum(2015,5,1,12,0,0):1:datenum(2015,5,4,12,0,0); % time of analysis
%time_lima  = datenum(2015,5,10,12,0,0):1:datenum(2015,5,13,12,0,0); % time of analysis
%2015,5,1
%time_lima  = datenum(2015,5,1,12,0,0):1:datenum(2017,5,30,12,0,0); % time of analysis
...
%runs=[79 81 117]; % aall
runs=[79 80 96 98 81]; % assh vs asst vs asf
runs=[81 98 96 80 79]; % part1
runs=[137 81 142 141 136 79 151]; % 7 days
...
colors={'r','b','k'}; % part 1
colors={'r','b','c','m','k','y','g','--b','--m','--r','--g','--c','--k','--y','r','b','m','c','k','y','g'}; % part 2
%colors={'r','k','y','g','--b','--m','--r','--g','--c','--k','--y','r','b','m','c','k','y','g'}; % part 3
%colors={'r','k'}; % chap 3
colors={'k','k' ,'m','c','b','r' ,'r'};
lstyle={'-','--','-','-','-','-' ,'--','-','-','-','-','-','-','-','-','-','-','-'};


% profiles
%parampr=[5 6]; % um vm tm sm ru rv rt rs
%parampr=[10 11]; % part1
parampr=[7 9 15 16]; % last fig m45 and argo
parampr=[7 9]; % last fig m45
%parampr=[14]; % ueof1 profile
%parampr=[3]; % t bias and rmsd
parampr=[17 15 18 16]; % argo
% time depth
paramtd=[7 11 12]; % u v t s du dv dt ds tc sc tcd scd
%paramtd=[1 7 8 11 12]; % part1
paramtd=[11]; % u v t s du dv dt ds tc sc tcd scd
% series
params=[9:14];
%params=[1 2 3 4 5 6 7 15]; % important
params=[2 3 4 6 8 10 11 12];
params=[17 18]; % divergence
%maps
paramm=[13];  % bias
...
variablespr={'serie_mod_u'   ,'serie_mod_v'    ,'serie_mod_t' ,'serie_mod_s',...
             ...
             'v_complex_an'  ,'serie_mod_ueof1','t_modcora'   ,'s_modcora',...
...
variablestd={'serie_mod_u' ,'serie_mod_v' ,'serie_mod_t'   ,'serie_mod_s',...
             ...
             't_modcora'   ,'s_modcora'   ,'t_modcora_diff','s_modcora_diff',...  12
             'serie_mod_w'}; 
...
variabless={'serie_mod_ssh','serie_corr_ssh','serie_rmsd_ssh',...
            ...
            'serie_rmsd_u' ,'serie_rmsd_v'  ,'serie_rmsd_t'  ,...
            'serie_rmsd_s' ,'serie_mod_div' ,'serie_bias_sst',...     15
            'serie_mod_dre','serie_modw'    ,'serie_modwre'};
...
[exptss,expt_namess,start_times] = expt_names_dates(1);
...
if calc_stats
    ...
    for ex=1:length(expts)
      ...
      serie_modw=nan(1,length(time_lima));
      serie_modwre=nan(1,length(time_lima));
      serie_mod_div=nan(1,length(time_lima));
      serie_mod_dre=nan(1,length(time_lima));
      ...
      serie_mod_w=nan(size(dep_mod,1),5,length(time_lima));
      ...
      for iobs=1:length(time_avgll)-1:length(time_lima)-length(time_avgll)+1
              ...
              if strncmp(expt,'free_',5)==1
                ...
                if exist(roms_nc)~=2
                  ...
                  for m=[1 2 3 4 5]
                    ...
                    wtd_mod(:,m)=nanmean(ncread(roms_nc,'w',[ilonm(m) ilatm(m) 1 1],[1 1 30  Inf]),4);
                  end
                  ...
                  ubar_mod=nanmean(u_mod(:,:,layerp),3);
                  vbar_mod=nanmean(v_mod(:,:,layerp),3);
                  wbar_mod=nanmean(w_mod(:,:,layerp),3);
                end
                ...
                if exist(roms_nc)~=2
                  ...
                  if length(time_mod)~=17 & length(time_mod)~=25 & length(time_mod)~=33 & length(time_mod)~=57
                    ...
                    w_mod=nanmean(ncread(roms_nc,'w',[1 1 1 nn(1)],[Inf Inf 30  nn(2)-nn(1)+1]),4);
                    ...
                    for m=[1 2 3 4 5]
                      ...
                      wtd_mod(:,m)=nanmean(ncread(roms_nc,'w',[ilonm(m) ilatm(m) 1 nn(1)],[1 1 30  nn(2)-nn(1)+1]),4);
                    end
                    % profiles (Argo, ctd, xbt)
                    for ii=1:length(cora_sufix1)
                      for iii=1:length(cora_sufix2)
                        ...
                        if exist(corasave)==2
                          ...
                          corak=corak+1;
                          load(corasave)
                          t_mo=nanmean(squeeze(ncread(roms_nc,'temp',[iloncs ilatcs 1 nn(1)],[1 1 Inf nn(2)-nn(1)+1])),2);
                          %t_mo=interp1(dep_mod(:,ilatm(m),ilonm(m)),t_mo,-tdepth(m));
                          t_modcora(:,corak)=t_mo;
                          s_mo=nanmean(squeeze(ncread(roms_nc,'salt',[iloncs ilatcs 1 nn(1)],[1 1 Inf nn(2)-nn(1)+1])),2);
                          %s_mo=interp1(dep_mod(:,ilatm(m),ilonm(m)),s_mo,-tdepth(m));
                          s_modcora(:,corak)=s_mo;
                          time_modcora(:,corak)=nanmean(time_mod(nn(1):nn(2)));
                          dept_modcora(:,corak)=squeeze(dep_mod(:,ilatcs,iloncs));
                        end
                      end
                    end
          ...
          serie_mod_w(:,:,i)=wtd_mod(:,:);

          serie_mod_sst(i)=nanmean(sst_mod(:));
          serie_bias_sst(i)=nanmean(sst_mod(:)-sst_avhrr(:));
          ...
          % divV=du/dx+dv/dy
          du=ubar_modnr(2:end,:)-ubar_modnr(1:end-1,:);
          dx=(1./grd.pm(:,2:end)-1./grd.pm(:,1:end-1))';
          dv=vbar_modnr(:,2:end)-vbar_modnr(:,1:end-1);
          dy=(1./grd.pn(2:end,:)-1./grd.pm(1:end-1,:))';
          ...
          serie_mod_div(i)=nanmean(nanmean(du./1999.67+dv./1999.67));
          serie_modw(i)=nanmean(nanmean(wbar_mod(:)));
          serie_modwre(i)=nanmean(nanmean(wbar_mod(70:111,89:107)));

          % rectagle around the mooring
          dure=du(70:111,89:107);
          dvre=dv(70:111,89:107);
          serie_mod_dre(i)=nanmean(nanmean(dure./1999.67+dvre./1999.67));
...
if plot_pr
 ...
 % Loop in the variables
  ...
  kk=0;
   ...
   % ploting markers first for legend
  figure('position',scrsz,'color',[1 1 1],'visible','on');  hold on;
  set(gca,'fontsize',16,'fontweight','bold')
  %plot(md_int,dept_t,'linestyle','none','marker',mks{ex},'MarkerFaceColor',colors{ex},'MarkerEdgeColor',colors{ex},'markersize',mss(ex))
  for vari=1:length(variablepr)
    ...
    for m=mooring
      ...
      mks={'d','.','s','^','p','.','s','^','p','.'};
      mks={'d','.','s','^','p','.','.','^','p','.'};
      mss=[10 ,24 ,12 ,9  ,12 ,24 ,1  ,9  ,12 ,1];
      subplot(1,length(variablepr).*length(mooring),mpr)
      hold on
      for ex=1:length(expts)
        plot(0,-2500,'linestyle',lstyle{ex},'linewidth',2,'color',colors{ex},'marker',mks{ex},'MarkerFaceColor',colors{ex},'MarkerEdgeColor',colors{ex},'markersize',mss(ex))
      end
...
%return
      ...
      for wts=wtss
        ...
        for it=1:length(times)
          ...
          for ex=1:length(expts)
            ...
            elseif strcmp(varib,'v_complex_co') || strcmp(varib,'v_complex_an') 
              ...
              %for i=50 % 1:size(um_obs,1)
                ...
                if m==3
                  ...
                  if sum(isnan(um_obs(i,is:ie)))<length(is:ie) & sum(isnan(u_mod(i,is:ie)))<length(is:ie)
            ...
            set(gca,'fontsize',12,'fontweight','bold')
            lett={'(a) ','(b) ','(c) ','(d) '};
            title([lett{mpr},vname,' ',meson{it}]) 

            if ~strcmp(varib,'serie_mod_ueof1') & ~strncmp(varib,'serie_rmsd_',11) & ~strncmp(varib,'v_complex_co',14) & ~strncmp(varib,'v_complex_an',14) & ~strncmp(varib,'serie_corr_',11)  & ~strncmp(varib,'t_modcora',9) & ~strncmp(varib,'s_modcora',9)
            ...
            elseif strncmp(varib,'t_modcora',9) || strncmp(varib,'s_modcora',9)
              ...
              [~,imax]=nanmin(depth(1,:));
              plot(modelo,depth(:,imax),'color',colors{ex},'linestyle',lstyle{ex},'linewidth',2)

              if strcmp(varib(end-4:end),'_diff')
                lege=[lege,{[expt_name,' std= ',num2str(nanstd(modelo),'%.2f')]}];
              else
          ...
          if strcmp(varib,'serie_mod_t') || strcmp(varib,'serie_rmsd_t') || strncmp(varib,'t_modcora',9) || strncmp(varib,'s_modcora',9)
            for ex=1:length(expts)
              eval(['md_data=data_md.ex',num2str(ex),'(:,',num2str(ex),');']);
              for mm=[3 4 5]
                if strncmp(varib,'s_modcora',9)
                  s_depth=nan(size(ts_depth)); 
                  s_depth([1,end-4],3)=ts_depth([1,end-4],3);
                  s_depth([1,6,end-3],4)=ts_depth([1,6,end-3],4);
                  s_depth([1,8,end],5)=ts_depth([1,8,end],5);
                  dept_t=s_depth(:,mm);
                else
                  dept_t=ts_depth(:,mm);
                end
                md_int=interp1(depth(:,1),md_data,dept_t);
                plot(md_int,dept_t,'linestyle','none','marker',mks{ex},'MarkerFaceColor',colors{ex},'MarkerEdgeColor',colors{ex},'markersize',mss(ex))
                if strcmp(varib,'serie_rmsd_t')
                  plot(irmsd(ex),0,'linestyle','none','marker',mks{ex},'MarkerFaceColor',colors{ex},'MarkerEdgeColor',colors{ex},'markersize',mss(ex))
                end
              end
            end
          end
          ...
          %legend(lege,'location','southeast','fontsize',11)
            lh=legend(lege,'location','best','fontsize',11);
          end

          if mpr==1
            ylabel('Depth (m)')
            xlabel('Temperature bias (^oC)')
            set(lh,'position',[0.0984955673758865        0.22458167794013         0.110937497485429        0.0954274327214621])
          elseif mpr==2
            xlabel('Temperature rmsd (^oC)')
            set(lh,'position',[0.379478726213745         0.22458167794013         0.120833330523844        0.0954274327214621])
          elseif mpr==3
            xlabel('Salinity bias (g/kg)')
            set(lh,'position',[0.568021276595744         0.22458167794013         0.110937497485429        0.0954274327214618])
          elseif mpr==4
            xlabel('Salinity rmsd (g/kg)')
            set(lh,'position',[0.793796102100269         0.22458167794013         0.120833330523843         0.095427432721462])
          end
          box('on')
  ...
  if plot_argo==1
    ...
    h=axes('position',[0.38 0.39 0.30 0.30]); % [left bottom width height]

    hold on
    m_proj('lambert','long', [173.8 max(lon_mod(:))],'lat',[-37.2 -33.4]);

    [cs,h]=m_contour(lon_mod,lat_mod,h_mod,[-1000 -200],'color',[.7 .7 .7],'linewidth',1);
    ...
    m_grid('xticklabels',[],'yticklabels',[],'fontname','helvetica','fontsize',18,'fontweight','bold');

    set(gca,'fontsize',18,'fontname','helvetica','fontweight','bold')


    %m_plot(lon_modo(3:5),lat_modo(3:5),'linewidth',2,'color',[0 0 1]) %#obs_cnames[0])
    m_plot(lonc_obs(3:5),latc_obs(3:5),'linewidth',2,'color','m') %#obs_cnames[0])
    for on=[3:5]
      m_plot(lonc_obs(on),latc_obs(on),'marker','.','markersize',10,'color','k') %#obs_cnames[0])
    ...
    for k=1:length(lon_corat)
      %m_text(lon_corat(k),lat_corat(k),num2str(k),'color',jet(7*k,:),'fontsize',10,'fontweight','bold');
      if k==5 || k==10 || k==7 || k==8 || k==23 || k==30
        m_text(lon_corat(k),lat_corat(k),num2str(k),'color','r','fontsize',10,'fontweight','bold');
      elseif k==23
        m_text(lon_corat(k),lat_corat(k),num2str(k),'color','b','fontsize',10,'fontweight','bold');
      else
        m_text(lon_corat(k),lat_corat(k),num2str(k),'color','k','fontsize',10,'fontweight','bold');
      end
    end

    i=2; m_plot([grd.corners(i-1,1) grd.corners(i,1)],[grd.corners(i-1,2) grd.corners(i,2)],'k')
    i=4; m_plot([grd.corners(i-1,1) grd.corners(i,1)],[grd.corners(i-1,2) grd.corners(i,2)],'k')
    m_plot([grd.corners(1,1) grd.corners(4,1)],[grd.corners(1,2) grd.corners(4,2)],'k')

    title('(e) Argo locations','fontsize',14,'fontweight','bold')

  end

  path_fig=['/scale_wlg_nobackup/filesets/nobackup/niwa00020/santanarc/figures/hikurangi/paper_2/'];
  figname=[path_fig,'','ch3_fig7_pr_argo.png'];
