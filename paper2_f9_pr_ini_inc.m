...
runs=[79 80 96 98 81]; % part1 
runs=[81 98 96 80]; % part1
runs=[137 81 142 141 136]; % 7 days

%runs=[91];
%runs=[80 81];
%runs=[80 96 98 81 122 123];
c=0
...
if calc_stats
    ...
    for ex=1:length(expts)
      ...
      for iobs=1:2:length(time_lima)-length(time_avgll)+1
 ...
 % Loop in the variables
  ...
  for vari=1:length(variablepr)
    ...
    for m=mooring
      mpr=mpr+1;
      mks={'d','.','s','^','p','.','s','^','p'};
      mss=[10 ,24 ,12 ,9  ,12 ,24 ,12 ,9  ,12];
      subplot(1,length(variablepr).*length(mooring),mpr)
      hold on
      for ex=1:length(expts)
        %plot(0,-2500,'-','linewidth',2,'color',colors{ex},'marker',mks{ex},'MarkerFaceColor',colors{ex},'MarkerEdgeColor',colors{ex},'markersize',mss(ex))
      end
    end
  end
...
%return
      ...
      for wts=wtss
        ...
        for it=1:length(times)
          ...
          for ex=1:length(expts)
            ...
            lett={'(a) ','(b) '};
            title([lett{mpr},'M',num2str(m),' ',vname,' ',meson{it}]) 
            ...
            elseif strcmp(varib,'serie_inc_t')
              ...
              %lege=[lege,{[expt_name,' mean= ',num2str(nanmean(modelo(:)),'%3.2e')]}];
              lege=[lege,{[expt_name,'']}];
              eval(['data_md.ex',num2str(ex),'(:,',num2str(ex),')=nanmean(modelo,2);'])
              depth=dep_modp;
            else
          ...
          plot([0 0],[-1800 0],'--k','linewidth',2)

          if m==4 % length(mooring)*length(variablepr)>2*3
            %legend(lege,'location','southoutside','fontsize',11)
            legend(lege,'location','east','fontsize',18)
          else
            legend(lege,'location','east','fontsize',18)
            %legend(lege,'location','best','fontsize',11)
          end
          
          xlabel('Mean temperature increment (^oC)')
          ylabel('Depth (m)')
  ...
  figname=[path_fig,'','ch3_fig9_ini_inc.png'];
